    ...
    */
    ajax: {
      ...
      _ajax: function(request) {
        ...
        if (_xhr) {
          Pollen.ajax._confXHR(_xhr, options, json, ajaxSuccess, ajaxError);
          ...
          _xhr.setRequestHeader("Accept", options.accept);

          _xhr.send(  _type == "post" ? options.data : null );
        }
      },
      _confXHR: function(_cxhr, options, json, ajaxSuccess, ajaxError) {
        ...
        var onreadystatechange = _cxhr.onreadystatechange = function() {
          ...
          if (_cxhr.readyState == 4) {
            ...
            if (_cxhr.status >= 200 && _cxhr.status < 300 || _cxhr.status === 304){
                success();
            }else {
                // We extract error from statusText
                // then normalize statusText and status for non-aborts
                var errorText = _cxhr.statusText;
                if ( !_cxhr.statusText || _cxhr.status ) {
                    _cxhr.statusText = "error";
                    if ( _cxhr.status < 0 ) {
                        _cxhr.status = 0;
                    }
                }
